...
export default function DeletePostulacionButton({ id }: Props) {
  ...
  async function handleDelete() {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setError(json.error ?? 'Error al eliminar la postulación');
        setLoading(false);
        return;
      }
      ...
      setError('Error de red al eliminar la postulación');
      setLoading(false);
    }
  }
      ...
      {dialog && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm">
          <div className="mx-4 w-full max-w-md rounded-xl border border-gray-200 bg-white p-6 shadow-2xl">
            <h3 className="text-lg font-bold text-red-600">Eliminar postulación</h3>
